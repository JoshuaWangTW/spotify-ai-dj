...
export type AlbumArtKind =
  | 'mountains' | 'sax' | 'violin' | 'orb'
  | 'piano' | 'sunset' | 'forest' | 'blues' | 'desert';
...
export default function AlbumArtwork({
  ...
  const palettes: Record<AlbumArtKind, { sky: [string, string]; land: [string, string]; sun: string }> = {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" style={{ borderRadius: radius, display: 'block' }}>
      <defs>
      ...
      <g clipPath={`url(#clip-${id})`}>
        ...
        {kind === 'mountains' && (
          <>
            <path d="M0 130 L25 105 L55 125 L85 95 L120 120 L155 100 L200 115 L200 200 L0 200 Z" fill={c.land[0]} opacity="0.5" />
            <path d="M0 150 L30 125 L60 145 L95 110 L130 140 L165 120 L200 135 L200 200 L0 200 Z" fill={`url(#land-${id})`} />
        ...
        {kind === 'orb' && (
          <>
            ...
            <ellipse cx="85" cy="84" rx="22" ry="12" fill="rgba(255,255,255,0.75)" transform="rotate(-22 85 84)" />
            <circle cx="100" cy="100" r="46" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.8" />
          </>
        )}
        {kind === 'violin' && (
          <>
        ...
        {kind === 'sax' && (
          <>
            ...
            <g transform="translate(100 100)">
              <path d="M-32 -50 C -28 -56 -16 -56 -10 -48 L -6 -38" stroke={c.land[0]} strokeWidth="6" fill="none" strokeLinecap="round" />
              <path d="M-6 -38 L 2 28 C 4 44 18 50 32 46" stroke={c.land[0]} strokeWidth="14" fill="none" strokeLinecap="round" />
              <ellipse cx="40" cy="44" rx="14" ry="8" fill={c.land[0]} transform="rotate(-10 40 44)" />
              <ellipse cx="40" cy="44" rx="9" ry="4.5" fill={c.land[1]} transform="rotate(-10 40 44)" />
            </g>
          </>
        )}
        {kind === 'sunset' && (
          <>
            ...
            <path d="M0 140 Q 50 120 100 140 Q 150 160 200 140 L 200 200 L 0 200 Z" fill={c.land[0]} opacity="0.75" />
            <path d="M0 165 Q 60 155 100 165 Q 140 175 200 165 L 200 200 L 0 200 Z" fill={c.land[1]} opacity="0.8" />
          </>
        )}
        {kind === 'desert' && (
          <>
            <circle cx="150" cy="60" r="22" fill={c.sun} opacity="0.9" />
            <path d="M0 140 Q 60 110 110 130 Q 160 150 200 125 L 200 200 L 0 200 Z" fill={c.land[0]} />
            <path d="M0 165 Q 50 140 100 160 Q 150 180 200 155 L 200 200 L 0 200 Z" fill={c.land[1]} opacity="0.85" />
          </>
        )}
        {kind === 'piano' && (
          <>
            ...
            {Array.from({ length: 8 }).map((_, i) => (
              <rect key={i} x={12 + i * 22} y="70" width="20" height="110" fill={c.sun} opacity={0.92} stroke={c.land[1]} strokeWidth="0.5" />
            ))}
          </>
        )}
        {kind === 'forest' && (
          <>
            ...
            {[{x:40,h:80},{x:75,h:110},{x:105,h:95},{x:138,h:120},{x:170,h:90}].map((t,i)=>(
              <path key={i} d={`M${t.x-16} 170 L${t.x} ${170-t.h} L${t.x+16} 170 Z`} fill={c.land[1]} opacity={0.7+(i%2)*0.2}/>
            ))}
          </>
        )}
        {kind === 'blues' && (
          <>
            {[60,90,120,150,180].map((y,i)=>(
              <path key={i} d={`M-10 ${y} Q 50 ${y-16} 100 ${y} T 210 ${y}`}
                stroke={c.land[i%2]} strokeWidth={2-i*0.2} fill="none" opacity={0.7-i*0.08}/>
            ))}
          </>
        )}
