...
import {
  IconChevronDown, IconHeart, IconHeartFilled, IconMore, IconNext, IconPause,
  IconPlay, IconPrev, IconRepeat, IconShuffle, IconSpark, IconSpeaker, IconList,
} from '../icons';
...
export default function NowPlayingModal({ onClose, onOpenCommentary }: Props) {
  const {
    activateBrowserDevice, deviceId, isPlaying, notice, playerReady,
    progressPercent, runPlayerCommand, status, track,
  ...
  const noticeTone =
    notice?.tone === 'success' ? 'border-emerald-300/60 bg-emerald-50 text-emerald-700'
    : notice?.tone === 'info' ? 'border-sky-200/50 bg-sky-50 text-sky-700'
    ...
    >
      ...
      <div className="flex items-center justify-center px-5 pt-3 pb-5">
        <div
          ...
          style={{ boxShadow: '0 14px 40px rgba(70,110,140,0.25), inset 0 1px 0 rgba(255,255,255,0.5)' }}
        >
          {cover ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={cover} alt="" className="h-full w-full object-cover" />
          ) : (
            <div className="h-full w-full" style={{ background: 'linear-gradient(135deg, #bae6fd, #7dd3fc)' }} />
          )}
        </div>
      </div>
      ...
      <div className="flex items-start gap-4 px-6 pb-1">
        <div className="flex-1 min-w-0">
          <h2 className="m-0 truncate text-[22px] font-bold tracking-tight text-slate-900">{title}</h2>
      ...
      <div className="px-6 pt-4">
        <div className="relative h-1 rounded-full bg-slate-300/60">
          <div
            ...
            style={{ width: `${progressPercent}%`, background: 'linear-gradient(90deg, #7dd3fc, #0284c7)' }}
          />
          <div
            className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white"
            style={{ left: `${progressPercent}%`, boxShadow: '0 1px 4px rgba(0,0,0,0.15), 0 0 0 1px rgba(2,132,199,0.5)' }}
          />
        </div>
        ...
        {status === 'device_inactive' && deviceId ? (
          ...
          >
            ...
            <span className="font-medium">
              {status === 'device_active' || status === 'ready' ? 'AI DJ Web Player' : 'No active device'}
            </span>
          </button>
        )}
      </div>
        ...
        <div className="px-5 pb-10">
          <div className="glass-card rounded-2xl p-3.5">
            <div className="mb-2 flex items-center justify-between">
              <div className="text-[12px] font-semibold uppercase tracking-wider text-slate-500">Up Next</div>
